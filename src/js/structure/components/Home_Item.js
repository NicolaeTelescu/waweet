...
function ItemRating({rating}) {
	...
	return (
		<div className="home__item__rating    home__item__border-animation">
			<div className="home__item__rating-triangle" style={myStyle}>
				<div className="home__item__rating-number">{ratingModified}</div>
			</div>
		</div>
	);
}

function ItemDetails({title, category, slug}) {
	return (
...
function EditItemButton({slug}) {
	return (
		<a href={"/item/edit/" + slug} className="home__item__button">
...
function DeleteItemButton({slug}) {
	...
	return (
		<form className="home__item__button" action={"/item/delete/" + slug} method="post" id={"deleteItem-" + slug} onClick={deleteElement}>
